    ...
    try{
        ...
        if(team_lead?.length > 0){
            ...
            workspace.members.forEach(member => {
                if(team_memebers.includes(member.user.email)){
        ...
        const projectWithMembers = await prisma.project.findUnique({
            ...
            include:{
                ...
                tasks: {include: {assignee: true, Comment: {include: {user: true}}}},
                owner: true
            }
        })
...
export const addMember = async(req,res) => {
    try{
        ...
        //Check if user is laready a member
        const existingMembers = project.members.find((member) => member.email === email)

        if(existingMembers){
            return res.status(400).json({message: "User is already a mener"});
        }
